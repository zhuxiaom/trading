from zenzic.strategies.pytorch.PatchTST.price.model import PatchTSTPrices
...
def load_dataset(dir, type, device, in_channels):
    path = os.path.join(dir, type + '.pkl')
    print(f"Load {type} dataset from '{path}'.")
    with open(path, 'rb') as file:
        data = pickle.load(file)
        x = data['x'][:, :, in_channels]
        date_x = data['date_x']
        y = data['y'][:, :, 3]  # close price
        date_y = data['date_y']
...
def find_st_trial(st_checkpoit_dir: str):
    # e.g. C:\Trading\PatchTSTPrices\2023-12-27\PatchTSTPrices-2024-01-01-18-17-39-789\0\checkpoints
...
def main():
    ...
    parser.add_argument('--in_channels', type=tuple, default=(0, 1, 2, 3))
    ...
    parser = PatchTSTPrices.add_model_args(parser)
    ...
    # data
    # ------------
    train_data = load_dataset(dir=args.input_dir, type='train', device='cuda:0', in_channels=args.in_channels)
    val_data = load_dataset(args.input_dir, type='val', device='cuda:0', in_channels=args.in_channels)
    ...
    if checkpoint_exists:
        model = PatchTSTPrices.load_from_checkpoint(checkpoint_file, configs=args)
    else:
        model = PatchTSTPrices(args)
    ...
    save_model = ModelCheckpoint(
        ...
        filename="PatchTSTPrices-{epoch:03d}-{val_loss:.6f}",
        save_top_k=3,
        mode="min",
