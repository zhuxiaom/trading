import torch
from zenzic.strategies.pytorch.autoformer.trade.models import AoT
...
def main():
    ...
    parser = AoT.add_model_args(parser)
    ...
    train_loader = DataLoader(
        Dataset(args.data_file, 'train', args.seq_len),
        batch_size=args.batch_size,
        num_workers=4,
        shuffle=True,
        persistent_workers=True)
    val_loader = DataLoader(
        Dataset(args.data_file, 'val', args.seq_len),
        batch_size=args.batch_size,
        num_workers=2,
        persistent_workers=True)
    ...
    save_model = ModelCheckpoint(
        ...
        filename="AoT-{epoch:03d}-{val_loss:.6f}",
        save_top_k=3,
        mode="min",
