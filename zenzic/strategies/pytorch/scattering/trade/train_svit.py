...
def main():
    ...
    parser = SViT.add_model_args(parser)
    ...
    train_loader = DataLoader(
        Dataset(args.data_file, 'train', args.seq_len, channel_first=True),
        batch_size=args.batch_size,
        num_workers=4,
        shuffle=True,
        persistent_workers=True)
    val_loader = DataLoader(
        Dataset(args.data_file, 'val', args.seq_len, channel_first=True),
        batch_size=args.batch_size,
        num_workers=2,
        persistent_workers=True)
    ...
    save_model = ModelCheckpoint(
        ...
        filename="SViT-{epoch:03d}-{val_loss:.6f}",
        save_top_k=3,
        mode="min",
